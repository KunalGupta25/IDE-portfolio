...
export function HomeClient({ profile, skills }: { profile: any, skills?: any }) {
  return (
    <div className="relative flex min-h-full w-full flex-col items-center text-gray-300">
      <ClientBootWrapper>
        <HomePageContent profile={profile} skills={skills} />
      </ClientBootWrapper>
    </div>
  );
}

function HomePageContent({ profile, skills }: { profile: any, skills?: any }) {
    ...
    <div className="relative w-full flex items-center justify-center py-8 px-4 min-h-full">
      ...
      >
        ...
        <div className={`flex-shrink-0 flex items-center justify-center ${
          ...
          <OrbitingLogosPositioned skills={skills} />
        </div>
      </div>
    </div>
  );
...
function OrbitingLogosPositioned({ skills }: { skills?: any }) {
  return (
    <div className="ide-responsive-logos">
      <OrbitingLogos skills={skills} />
    </div>
  );
}
